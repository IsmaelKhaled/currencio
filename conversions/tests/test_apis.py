...
class ConversionAPITestCase(TestCase):
    ...
    def test_api_rejects_from_currency_outside_choices(self):
        ...
        response = client.post('/api/convert/', payload, format="json")
        self.assertEqual(response.status_code, 400)

    def test_api_rejects_to_currency_outside_choices(self):
        conversion_models.DateExchangeRates.objects.create(rates=self.rates)
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION=f'Token {self.token.key}')
        payload = {
    ...
    def test_response_data(self):
        ...
        expected_response_data = {
            ...
            'converted_amount': self.amount * float(self.rates.get('EGP'))
        }
