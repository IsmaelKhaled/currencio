...
class ConversionCountTestCase(TestCase):
    def setUp(self):
    ...
    def test_count_is_created_if_not_exists(self):
        self.assertEqual(conversion_models.ConversionCount.objects.count(), 0)
        conversion_models.DateExchangeRates.objects.create(rates=self.rates)
        client = APIClient()
        ...
        client.post('/api/convert/', payload, format="json")
        self.assertEqual(conversion_models.ConversionCount.objects.count(), 1)

    def test_count_increases_upon_request(self):
        conversion_models.DateExchangeRates.objects.create(rates=self.rates)
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION=f'Token {self.token.key}')
        payload = {
        ...
        for i in range(TIMES_API_CALLED):
            client.post('/api/convert/', payload, format="json")
        self.assertEqual(
            conversion_models.ConversionCount.objects.first().count, TIMES_API_CALLED)


class DateExchangeRatesTestCase(TestCase):
    def setUp(self):
        self.from_currency = "EUR"
        self.to_currency = "EGP"
        self.amount = 2.0
    ...
    # def test_rates_is_created_if_not_exists(self):
    ...
    #     client.post('/api/convert/', payload, format="json")
    #     self.assertEqual(
    #         conversion_models.DateExchangeRates.objects.count(), 1)

    def test_rates_is_not_created_if_exists(self):
        conversion_models.DateExchangeRates.objects.create(rates=self.rates)
        client = APIClient()
        client.credentials(HTTP_AUTHORIZATION=f'Token {self.token.key}')
        payload = {
